...
class Twiggy
{
    ...
    protected function loadChunk($name, $row = array())
    {
        ...
        if (!empty($this->config['nestedChunkPrefix']) AND strpos($content,
                '<!--' . $this->config['nestedChunkPrefix']) !== false
        ) {
            ...
            try {
                ...
                if (!empty($loaders)) {
                    ...
                    foreach ($loaders as $loader) {
                        if ($loaderClass = $this->modx->loadClass('TwiggyLoader' . $loader, $pathLoaders, false,
                            true)
                        ) {
                ...
                if ($sandbox) {
                    ...
                    if (!$sandboxConfig = $this->getCache($options)) {
                        $sandboxTags = $this->explodeAndClean($this->getOption('sandbox_tags', $this->config, '',
                            true));
                        $sandboxFilters = $this->explodeAndClean($this->getOption('sandbox_filters', $this->config, '',
                            true));
                        $sandboxMethods = $this->modx->fromJSON($this->getOption('sandbox_methods', $this->config, "{}",
                            true));
                        $sandboxProperties = $this->modx->fromJSON($this->getOption('sandbox_properties', $this->config,
                            "{}", true));
                        $sandboxFunctions = $this->explodeAndClean($this->getOption('sandbox_functions', $this->config,
                            '', true));
                        $sandboxConfig = array(
                            'tags'       => $sandboxTags,
                ...
                if (!empty($extensions)) {
                    ...
                    foreach ($extensions as $extension) {
                        if ($extensionClass = $this->modx->loadClass('TwiggyExtension' . $extension, $pathExtensions,
                            false, true)
                        ) {
    ...
    public function clearTwiggyCache()
    {
        ...
        return $this->modx->cacheManager->deleteTree($folder,
            array('deleteTop' => true, 'extensions' => array('.string.php', '.cache.php', '.php')));
    }
